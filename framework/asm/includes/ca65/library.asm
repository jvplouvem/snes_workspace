...
; spr_sz = sprite size, 0 or 2
	...
	sta a:oam_lo_buffer, y
	lda spr_y ;y
	sta a:oam_lo_buffer+1, y
	lda spr_c ;tile
	sta a:oam_lo_buffer+2, y
	lda spr_a ;attribute
	sta a:oam_lo_buffer+3, y
...
;the high byte holds the X 9th bit
	sep #$20 ;a8
	sta a:oam_lo_buffer, x
;keep that high byte 9th x
	iny
	lda [temp1], y ;y byte
	clc
	adc spr_y	
	sta a:oam_lo_buffer+1, x
	iny
	lda [temp1], y ;tile
	sta a:oam_lo_buffer+2, x
	iny
	lda [temp1], y ;attributes
	sta a:oam_lo_buffer+3, x
	iny
	lda [temp1], y ;size
	iny
	sta spr_h
